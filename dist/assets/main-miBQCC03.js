import{T as a,i as d,C as n,F as i,t as s,a as r,H as c,r as x}from"./index-Bln418yw.js";window.__WC_TEMPLATES__=a;d("");const m=document.getElementById("app"),b=`
...
<section class="max-w-7xl mx-auto px-4 mb-16">
  ...
  <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    ${n.map(t=>{const l=a.filter(o=>o.cat===t.id).length;return`<a href="categories.html?cat=${t.id}" class="wc-card p-5 hover:-translate-y-1 group">
        <div class="text-3xl mb-2">${t.icon}</div>
        <h3 class="font-semibold">${t.name}</h3>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">${l} 个模板</p>
      </a>`}).join("")}
  </div>
</section>`,g=`
<section class="max-w-7xl mx-auto px-4 mb-16">
...
<section class="max-w-7xl mx-auto px-4 mb-16">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
    ${c.map(t=>s(r(t),{from:"home"})).join("")}
  </div>
</section>`;m.innerHTML=b+p+g+h;x();const e=document.getElementById("hero-search");e.addEventListener("keydown",t=>{t.key==="Enter"&&e.value.trim()&&(location.href="categories.html?q="+encodeURIComponent(e.value.trim()))});
