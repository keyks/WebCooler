import{T as m,i as p,b as r,a as c,r as g,t as h}from"./index-Bln418yw.js";window.__WC_TEMPLATES__=m;p("workbench");const x=document.getElementById("app"),n=r.get(),f=(n.favs||[]).map(c).filter(Boolean),v=(n.history||[]).map(c).filter(Boolean),d=n.shares||[];function l(e,t,s){return t.length?`<div class="mb-10"><h2 class="text-xl font-bold mb-4">${e} <span class="text-sm font-normal text-slate-400">(${t.length})</span></h2>
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">${t.map(a=>h(a,{fav:!0,from:"workbench"})).join("")}</div></div>`:`<div class="mb-10"><h2 class="text-xl font-bold mb-4">${e}</h2><p class="text-slate-400 text-sm">${s}</p></div>`}function y(e){return`
...
<section class="max-w-7xl mx-auto px-4 py-8">
  ...
  ${l("⭐ 我的收藏",f,"还没有收藏，去模板详情页点击收藏吧。")}
  ${l("🕘 浏览历史",v,"暂无浏览记录。")}
  ${i()}
</section>`;g();function u(){const e=r.get();d.length=0,(e.shares||[]).forEach(s=>d.push(s));const t=document.querySelector("#app .mb-12");if(t){const s=document.createElement("div");s.innerHTML=i(),t.replaceWith(s.firstElementChild),b()}}function b(){document.querySelectorAll("[data-del]").forEach(e=>e.onclick=()=>{const t=e.dataset.del,s=r.get();s.shares=(s.shares||[]).filter(a=>a.id!==t),r.set({shares:s.shares}),u()}),document.querySelectorAll("[data-use]").forEach(e=>e.onclick=()=>{var s;const t=d.find(a=>a.id===e.dataset.use);t&&((s=navigator.clipboard)==null||s.writeText(t.code||"").catch(()=>{}),alert("已复制「"+t.name+"」的代码到剪贴板，可直接粘贴使用。"))}),document.querySelectorAll("[data-copy]").forEach(e=>e.onclick=()=>{var s;const t=d.find(a=>a.id===e.dataset.use);t&&((s=navigator.clipboard)==null||s.writeText(t.code||"").catch(()=>{}),alert("代码已复制。"))})}document.getElementById("ns-add").onclick=()=>{const e=document.getElementById("ns-name").value.trim(),t=document.getElementById("ns-type").value,s=document.getElementById("ns-desc").value.trim(),a=document.getElementById("ns-code").value;if(!e)return alert("请填写卡片名称");if(!a.trim())return alert("请粘贴你的布局 / 元素代码");const o=r.get();o.shares=o.shares||[],o.shares.unshift({id:"s"+Date.now(),name:e,type:t,desc:s,code:a,preview:"",at:Date.now()}),r.set({shares:o.shares}),u()};b();
