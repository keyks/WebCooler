import{S as H,Y as W,m as _}from"./index-DrRY-Jl8-612mL4fq.js";function I(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function O(s){let t=I(s);return t=t.replace(/(&lt;!--[\s\S]*?--&gt;)/g,'<span class="tok-comment">$1</span>'),t=t.replace(/(&lt;\/?)([\w-]+)/g,'$1<span class="tok-tag">$2</span>'),t=t.replace(/([\w-]+)(=)(&quot;[^&]*&quot;|"[^"]*")/g,'<span class="tok-attr">$1</span>$2<span class="tok-string">$3</span>'),t}function R(s){let t=I(s);return t=t.replace(/(\/\*[\s\S]*?\*\/)/g,'<span class="tok-comment">$1</span>'),t=t.replace(/([\w-]+)(\s*:)/g,'<span class="tok-attr">$1</span>$2'),t=t.replace(/(:\s*)([^;{}]+)(;)/g,'$1<span class="tok-string">$2</span>$3'),t=t.replace(/@[\w-]+/g,r=>`<span class="tok-keyword">${r}</span>`),t}function P(s){let t=I(s);return t=t.replace(/(\/\/[^\n]*)/g,'<span class="tok-comment">$1</span>'),t=t.replace(/('[^']*'|"[^"]*"|`[^`]*`)/g,'<span class="tok-string">$1</span>'),t=t.replace(/\b(const|let|var|function|return|if|else|for|while|class|new|import|export|from|await|async|document|window|this)\b/g,'<span class="tok-keyword">$1</span>'),t=t.replace(/\b(\d+\.?\d*)\b/g,'<span class="tok-num">$1</span>'),t=t.replace(/([\w-]+)(\s*\()/g,'<span class="tok-fn">$1</span>$2'),t}function A(s,t){return t==="html"?O(s):t==="css"?R(s):t==="js"?P(s):I(s)}function U(s,t){return`
  (function(){
  ...
  `}function j(s,{html:t,css:r,js:o,cat:f="",id:g=""},{autoDemo:i=!0,speed:l=1}={}){const u=document.createElement("iframe");u.className="w-full h-full border-0",u.setAttribute("tabindex","-1"),u.setAttribute("sandbox","allow-scripts allow-same-origin"),s.innerHTML="",s.appendChild(u);const p=u.contentDocument,m=`<!DOCTYPE html><html><head><meta charset="utf-8">
...
<script>${o||""}<\/script>
</body></html>`;if(p.open(),p.write(m),p.close(),i){const d=p.createElement("script");d.textContent=U(f,g),p.body.appendChild(d)}return u}function z(s,t){try{s&&s.contentWindow&&(s.contentWindow.__wcSpeed__=t)}catch{}}function L(s,t){if(!s||!s.contentDocument)return;const r=s.contentDocument;let o=r.getElementById("wc-override");o||(o=r.createElement("style"),o.id="wc-override",r.head.appendChild(o));const{size:f=1,x:g=0,y:i=0,c1:l,c2:u,bg:p,radius:m}=t;let d=`#wc-root{transform:scale(${f}) translate(${g}px,${i}px);transform-origin:center center;transition:transform .25s}`;p&&(d+=`body{background:${p} !important}`),m&&(d+=`*[class]{border-radius:${m}px !important}`),l&&(d+=`
:root{--wc-c1:${l}}`),u&&(d+=`
:root{--wc-c2:${u}}`),l&&(d+=`
.wc-root *{}
[class*="b"]{background-color:${l} !important}
button,.rp,.ring,.star.on{background-color:${l} !important}`),o.textContent=d}async function D(s){try{return await navigator.clipboard.writeText(s),!0}catch{const t=document.createElement("textarea");return t.value=s,document.body.appendChild(t),t.select(),document.execCommand("copy"),document.body.removeChild(t),!0}}function J(s,t={}){const{size:r=1,x:o=0,y:f=0,radius:g=12,c1:i,c2:l,bg:u,speed:p=1}=t;let m=s.html||"";const d=`transform:scale(${r}) translate(${o}px,${f}px);transform-origin:center center;`;/id=["']wc-root["']/.test(m)?m=m.replace(/(<[^>]*id=["']wc-root["'][^>]*?)>/i,`$1 style="${d}">`):m=`<div id="wc-root" style="${d}">${m}</div>`;let S=s.css||"",w=`
/* WebCooler 实时参数覆盖 */
`;u&&(w+=`body{background:${u} !important}
`),i&&(w+=`:root{--wc-c1:${i}}
...
`+x),{html:m,css:S,js:x}}function N(s=""){const t=[],r=/#[0-9a-fA-F]{3,8}\b/g;let o;for(;o=r.exec(s);){const i=o[0].toLowerCase();!t.includes(i)&&i!=="#fff"&&i!=="#ffffff"&&i!=="#000"&&i!=="#000000"&&t.push(i)}const f=[],g=/(?:width|height|padding|font-size|max-width)\s*:\s*(\d+)px/g;for(;o=g.exec(s);)f.push(parseInt(o[1],10));return{colors:t.slice(0,4),maxSize:f.length?Math.max(...f):200}}const F=new URLSearchParams(location.search),E=F.get("back"),C=F.get("from");let $="category";E&&/(^|\/)index\.html/.test(E)?$="":/categories\.html/.test(E||"")?$="category":/workbench\.html/.test(E||"")?$="workbench":/downloads\.html/.test(E||"")?$="downloads":C==="home"?$="":(C==="workbench"||C==="fav")&&($="workbench");H($);const q=document.getElementById("app"),K=new URLSearchParams(location.search).get("id"),n=W(K);if(!n)q.innerHTML='<div class="max-w-3xl mx-auto px-4 py-20 text-center"><h1 class="text-2xl font-bold mb-4">模板未找到</h1><a href="categories.html" class="text-brand-600">返回模板库</a></div>';else{let s=function(){const e=J(n,b),a=(c,h,y)=>{const v=document.getElementById(c);v&&(v.querySelector("pre").innerHTML=A(y,h))};n.html&&a("code-html","html",e.html),n.css&&a("code-css","css",e.css),n.js&&a("code-js","js",e.js)},t=function(){const e=a=>{const c=document.querySelector(a);return c?c.innerText:""};return{html:n.html?e("#code-html pre"):"",css:n.css?e("#code-css pre"):"",js:n.js?e("#code-js pre"):"",cat:n.cat,id:n.id}},r=function(){const e=t(),a=document.getElementById("auto-demo");a&&(a.checked=!1),x=j(w,e,{autoDemo:!1}),L(x,b)};const o=["index.html","categories.html","workbench.html","downloads.html"],f=e=>{if(!e||e.startsWith("//")||e.includes("://"))return!1;const a=e.replace(/^\/+/,"");return o.some(c=>a===c||a.startsWith(c+"?"))},g=E,i=C;let l;if(g&&f(g))l=g;else switch(i){case"home":l="index.html";break;case"workbench":case"fav":l="workbench.html";break;default:l=n.cat?`categories.html?cat=${n.cat}`:"categories.html"}_.addHistory(n.id);const u=_.isFav(n.id),p=N(n.css),m=p.colors[0]||"#2f83ff",d=p.colors[1]||"#8b5cf6";q.innerHTML=`
  <section class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mt-3">
      ...
      <div class="flex gap-2">
        <button id="fav" class="px-4 py-2 rounded-lg text-sm border ${u?"border-amber-400 text-amber-500":"border-slate-200 dark:border-slate-700"}">${u?"★ 已收藏":"☆ 收藏"}</button>
        ...
        <div class="control-panel">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label class="text-xs text-slate-600 dark:text-slate-300">主元素颜色
              <input type="color" id="p-c1" value="${m}" class="wc-color w-full mt-1 h-8">
            </label>
